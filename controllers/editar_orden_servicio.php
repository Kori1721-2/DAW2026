...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $sql = "UPDATE orden_servicio SET id_servicio = ?, fecha = ?, cliente = ?, estado = ?, observaciones = ? WHERE id_orden = ?";
        $stmt = $conexion->prepare($sql);
        $stmt->execute([$id_servicio, $fecha, $cliente, $estado, $observaciones, $id_orden]);
